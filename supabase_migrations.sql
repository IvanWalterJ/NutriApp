...
-- ───────────────────────────────────────────────────────────────────────────
-- Formularios públicos pre-consulta (Sprint 3 — Fase 3.2).
--
-- Cada formulario tiene un slug único (ej. "feria-pampa-jun26") que se
-- comparte por link/QR. Los pacientes lo completan SIN auth — Supabase RLS
-- permite INSERT anónimo sobre form_responses si el form sigue vivo.
--
-- Después, las nutricionistas ven las respuestas en una bandeja y las
-- promueven a pacientes reales.
-- ───────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.forms (
  ...
  title       text NOT NULL,
  description text,
  -- Definición declarativa de los campos del form. Estructura:
...
CREATE TABLE IF NOT EXISTS public.form_responses (
  ...
  -- Snapshot de los datos cargados por el paciente. Las keys matchean
  -- fields[i].key del form que las originó.
  data          jsonb NOT NULL,
...
-- RLS forms: lectura pública sólo de los activos y no vencidos (para que el
-- formulario público pueda traer el título/campos por slug sin auth).
-- Inserción/edición/borrado sólo para autenticados.
...
-- RLS form_responses: INSERT público (paciente sin auth puede enviar
-- respuestas), pero SELECT/UPDATE/DELETE sólo para nutricionistas autenticadas.
...
CREATE POLICY "Anyone can submit form responses"
  ...
  TO anon, authenticated
  WITH CHECK (
    -- El form referenciado tiene que estar activo y no vencido al momento del submit.
    EXISTS (
